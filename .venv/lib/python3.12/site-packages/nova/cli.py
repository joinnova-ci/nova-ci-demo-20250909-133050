...
def _generate_ci_diff_summary(state: AgentState, repo_path: Path) -> None:
    ...
    diff_summary.append("<summary>Click to see beautiful code transformations ✨</summary>")
    diff_summary.append("")
...
def fix(
    ...
    if ci_mode:
        ...
        if not quiet:
            console.print(f"[green]Nova CI-Rescue[/green] 🚀 [dim](CI Mode)[/dim]")
    elif not quiet:
    ...
    if not quiet:
        ...
        if ci_mode:
            console.print(f"[dim]CI Mode: Direct commits • Patch fixes • PR integration[/dim]")
        console.print()
    ...
    try:
        with nova_lock(repo_path, wait=False):
            ...
            # Create the nova-fix branch (allow override) - skip in CI mode
            if ci_mode:
            ...
            while state.increment_iteration():
                ...
                patch_approved, review_reason = llm_agent.review_patch(patch_diff, state.failing_tests, state)
        ...
        if success and state and branch_name and git_manager:
            try:
                ...
                if ci_mode:
                    console.print("\n[green]✨ CI Mode: Fixes committed successfully![/green]")
                    console.print("[dim]• Direct commits to PR branch[/dim]")
                    console.print("[dim]• GitHub Actions will update PR status[/dim]")
                ...
                pr_gen = PRGenerator(repo_path, verbose=not quiet)
                ...
                if pr_gen.check_pr_exists(branch_name):
                    ...
                    if num_patches > 0:
                        try:
                            ...
                            if verbose:
                                console.print(f"[yellow]⚠️  Could not list changed files: {e}[/yellow]")
