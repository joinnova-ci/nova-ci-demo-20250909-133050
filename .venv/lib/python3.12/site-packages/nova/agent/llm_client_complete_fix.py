...
IMPORTANT NOTES:
- Focus on whether the implementation will make the tests PASS, not on theoretical best practices
- If tests expect specific exception types (e.g., ValueError vs ZeroDivisionError), the implementation should match what the tests expect
- Do not reject based on assumptions about what tests "might" expect - only reject if you can identify specific test failures

BE STRICT BUT PRACTICAL: Approve if the patch will make ALL tests pass, even if the implementation isn't theoretically perfect.
