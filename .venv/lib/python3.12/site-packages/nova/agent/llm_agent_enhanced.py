...
class EnhancedLLMAgent:
    ...
    def __init__(self, repo_path: Path, verbose: bool = False):
        ...
        self.llm = LLMClient(verbose=verbose)  # Use the unified LLM client
    ...
    def generate_patch(self, failing_tests: List[Dict[str, Any]], iteration: int, plan: Dict[str, Any] = None, critic_feedback: Optional[str] = None, state=None) -> Optional[str]:
        ...
        try:
            ...
            # Use demo mode max tokens if available
            max_tokens = 100000  # Default large limit
            if state and hasattr(state, 'demo_mode') and state.demo_mode:
                from nova.config import get_settings
                settings = get_settings()
                max_tokens = settings.demo_mode_max_tokens
    ...
    def review_patch(self, patch: str, failing_tests: List[Dict[str, Any]], state=None) -> Tuple[bool, str]:
        """
        ...
        Args:
            ...
            state: Optional agent state for demo mode detection
            
        Returns:
            Tuple of (approved: bool, reason: str)
        """
        from nova.tools.safety_limits import load_safety_limits, validate_patch_safety
        from nova.config import get_settings
        
        # Load safety limits (could be customized from config in the future)
        limits = load_safety_limits()
        
        # If in demo mode, use relaxed limits
        if state and hasattr(state, 'demo_mode') and state.demo_mode:
            settings = get_settings()
            limits.max_lines_per_patch = settings.demo_mode_max_lines
            limits.max_lines_per_file = settings.demo_mode_max_lines
        
        # Override enforcement based on instance attribute if available
        if hasattr(self, 'enforce_safety_limits'):
            limits.enforce_limits = self.enforce_safety_limits
        
        # Detect if this is whole file mode
        whole_file_mode = "FILE_REPLACE:" in patch
        
        # Use the centralized safety validation
        is_safe, error_msg = validate_patch_safety(patch, limits, whole_file_mode)
        
        if not is_safe and error_msg:
            # If limits are enforced, this is a hard stop
        ...
        try:
            ...
            if self.verbose:
                print("[Nova Debug - Critic] Calling LLM with temperature=0.1, max_tokens=100000")
            response = self.llm.complete(
                system=system_prompt,
                user=user_prompt,
                temperature=0.1,
                max_tokens=100000  # Increased to allow fuller responses
            )
            ...
            if response:
                ...
                if self.verbose:
                    print("[Nova Debug - Critic] WARNING: Empty response from LLM!")
            ...
            if not critic_feedback:
                if self.verbose:
                    print("[Nova Debug - Critic] Empty critic response, checking auto-approval criteria...")
            ...
            if self.verbose:
                print("[Nova Debug - Critic] Using raw response as feedback")
